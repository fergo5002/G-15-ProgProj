...
void mouseWheel(MouseEvent event) {
  int maxScroll = max(0, table.getRowCount() * 25 - (SCREENY - 150));
...
class DropdownWidget {
  ...
  void display() {
    ...
    if (expanded) {
      ...
      int maxDropdownHeight = min(options.size() * h, SCREENY - y - h); // Prevent going off-screen
